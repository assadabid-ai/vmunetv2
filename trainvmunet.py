...
class VMUNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            ...
            return torch.relu(out_0 + out_1)  #  [2, 1, 256, 256]
        else:
            if self.num_classes == 1: return torch.relu(seg_outs[-1])
...
def train_base_model(suffix, checkpoint=None): #encoder_name, encoder_weights, decoder_attention_type
    ...
    model = VMUNetV2(input_channels=in_channels, num_classes=OUTPUT_CHANNELS)
    ...
    if checkpoint!=None:
        s2s1_model = SentinelModel.load_from_checkpoint(model=model, checkpoint_path=checkpoint, 
                                                    mean_agb=mean_agb, std_agb=std_agb,
                                                    lr=0.0005, wd=0.0001)
    else:
